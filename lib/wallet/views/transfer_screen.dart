...
class TransferScreen extends StatefulWidget {
  ...
  final Accounts targetAccount;
  const TransferScreen(
      {Key? key, required this.account, required this.targetAccount})
      : super(key: key);
...
class _TransferScreenState extends State<TransferScreen> {
  ...
  void initState() {
    ...
    controller.accountController.text = widget.targetAccount.id.toString();
    controller.nameController.text = widget.targetAccount.accountName;
    controller.amountController.text = "";
    controller.swapAmountController.text = "";
    controller.amountWithoutFeeController.text = "";

    setState(() {
      controller.account = widget.account;
      controller.loadData();
      controller.targetCurrency = widget.targetAccount.currency;
      if (widget.account.currency == widget.targetAccount.currency) {
        currency = widget.targetAccount.currency == "USD" ? " USD" : " Gold";
        title = "Transfer " + currency;
      } else {
        currency = widget.targetAccount.currency == "USD" ? " USD" : " Gold";
        title = "Swap to " + currency;
        isSwap = true;
      }
      // print("BBBBB");
            ...
            body: Padding(
              ...
              child: SingleChildScrollView(
                child: Column(
                  children: [
                    icon.buttonIcon(
                        widget.account.currency == widget.targetAccount.currency
                            ? Config.codeTransfer
                            : Config.codeSwap,
                        color: color),
  ...
  Widget sourceAccountField() {
    return TextFormField(
      ...
      decoration: InputDecoration(
          ...
          filled: true,
          fillColor: readOnly,
          hintText: "Account Number " + widget.targetAccount.currency,
          enabledBorder: outlineInputBorder,
          focusedBorder: outlineInputBorder,
          border: outlineInputBorder,
          contentPadding: FxSpacing.all(16),
